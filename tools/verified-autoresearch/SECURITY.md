...
Evaluator and guard commands never run under the normal macOS user. The controller copies only tracked regular files into a disposable snapshot and mounts that snapshot read-only inside a Colima Linux VM/Docker container. The container has no network, runs as UID/GID 65534, has a read-only root filesystem, no additional capabilities, `no-new-privileges`, no Docker socket, a disposable no-exec `/tmp`, and CPU, RAM, PID, open-file, file-size, output and wall-time limits. A detached, bounded host watchdog kills the named disposable container even if the controller is terminated with `SIGKILL`; this path is covered by a live Colima test. An exclusive controller lock serializes each campaign, and exact tracked-file fingerprints are checked before and after every candidate guard and evaluation. A durable pending-transaction journal reconciles interrupted commits with the fsynced ledger before another run can proceed. The model process remains outside the container and is unloaded after each proposal.
