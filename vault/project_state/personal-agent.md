...
headline: Aire source advanced to `36d769f` with durable work receipts, but `3556088` remains the last physically accepted iPhone candidate and the private tailnet route is currently unavailable.
status: building
updated: 2026-08-14
role: project-state
---
...
- The exact `36d769f` source candidate needs an immutable post-commit verification receipt, installation on the physical iPhone and direct rendered-screen proof of the new Work Receipt surface before it can supersede `3556088` as the accepted candidate.
- The Tailscale daemon and exact private Serve route are unavailable at the latest live check; durable phone-to-Mac connectivity remains blocked until both are restored and verified.

## Next verified milestones

1. Keep clean local source candidate `36d769f` in `IrelandGPT-aire-production-ready` as the mechanically enforced mobile source of truth, while retaining `3556088` as the last physically accepted candidate until the newer receipt work passes the full device gate.
2. Restore the Tailscale daemon and reverify the exact private Serve route before treating the persisted endpoint as available; do not replace it with an ephemeral tunnel or LAN address.
3. Produce an immutable post-commit verification receipt for `36d769f`, install that exact candidate on the physical iPhone and capture direct rendered-screen evidence of the Work Receipt surface.
...
7. Preserve the verified separation between core connection/authentication failures and scoped Work refresh failures.
