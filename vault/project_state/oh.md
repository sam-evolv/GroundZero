...
**Care authentication hardening, 2026-08-01:** Migration 077 and the local distributed limiter proof pass. Multi-home selection, hidden demo/admin removal and exact-installation authority reached independent PASS, after which Sam explicitly authorized a dedicated controlled test relationship and an isolated immutable preview without alias movement. A separate controlled auth identity and clearly synthetic active installation were created under the internal OpenHouse tenant without changing customer records. Controlled local testing exposed and fixed a content API production-column mismatch, API and UI truthfulness bugs that treated every unknown system as Solar PV, and an authenticated middleware open redirect via decoded backslashes. Home and Assistant now use explicit heat-pump, solar and unknown branches; unknown state remains unavailable instead of inventing equipment or generation. The repeated read and operational matrices pass login, durable cookie, exact-home page/API access, foreign denial, assistant, pending service request, alert dismissal, logout and post-logout denial; marked records and limiter buckets were cleaned. The current clean 38-file commit is `280a8349b6df8dbe1a4dc6ee8c6ae2002380c655`, patch SHA-256 `2b7d87cf8d86afcf0d0dfc344bcf21079760cf701587a4ea274af814a2ef5647`; four smokes, TypeScript, production build, redirect/UI regressions, diff check and secret scan pass, with final review `deleg_e4db05e7` pending before preview deployment. Nothing was pushed, merged, deployed or aliased; YC reviewer production remains unchanged. See [[briefs/2026-08-01-openhouse-care-password-and-authority-hardening]].
