...
**Care authentication hardening, 2026-08-01:** Migration 077 and the local distributed limiter proof pass. Multi-home selection, hidden demo/admin removal and exact-installation authority reached independent PASS, after which Sam explicitly authorized a dedicated controlled test relationship and an isolated immutable preview without alias movement. A separate controlled auth identity and clearly synthetic active installation were created under the internal OpenHouse tenant without changing customer records. Controlled local testing exposed and fixed a content API production-column mismatch plus a truthfulness bug that treated every unknown system as Solar PV. The repeated read matrix now passes login, durable cookie, exact-home page plus manifest/telemetry/content/conversations/service-record reads, foreign page/API denial, logout and post-logout denial. The synthetic-only operational matrix also passes assistant, pending service request and empty alert dismissal with foreign denial; marked records and limiter buckets were cleaned. The definitive clean 37-file commit is `26141f0fca0b9ef53ab68d3c9371cf02cc87efb7`, patch SHA-256 `6c350bd3a0c7ac3cb1ea30181e35c0c8dd8d3138a96cb7665797767cd8bfcbcb`; four smokes, TypeScript, production build, diff check and secret scan pass, with final review `deleg_58c3d291` pending before preview deployment. Nothing was pushed, merged, deployed or aliased; YC reviewer production remains unchanged. See [[briefs/2026-08-01-openhouse-care-password-and-authority-hardening]].
