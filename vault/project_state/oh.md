...
**Care authentication hardening, 2026-08-01:** Migration 077 and the local distributed limiter proof pass. Multi-home selection, hidden demo/admin removal and exact-installation authority reached independent PASS, after which Sam explicitly authorized a dedicated controlled test relationship and an isolated immutable preview without alias movement. A separate controlled auth identity and clearly synthetic active installation were created under the internal OpenHouse tenant without changing customer records. The first controlled local production-build matrix exposed a real content API mismatch with nonexistent production columns; the route was corrected to inspected live fields and the repeated matrix now passes login, durable cookie, exact-home page plus manifest/telemetry/content/conversations/service-record reads, foreign page/API denial, logout and post-logout denial. The amended clean 37-file commit is `3ad9cea6177a47dcde6fe2326cc3703e6419c405`, patch SHA-256 `db97f58f4d7d46365b8c29f3d528b76552ce29cd9979fd675ccba99ae376f23e`; four smokes, TypeScript, production build, diff check and secret scan pass, with final review `deleg_912c9eab` pending before preview deployment. Nothing was pushed, merged, deployed or aliased; YC reviewer production remains unchanged. See [[briefs/2026-08-01-openhouse-care-password-and-authority-hardening]].
