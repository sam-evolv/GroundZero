...
**Care authentication hardening, 2026-08-01:** Migration 077 and the local distributed limiter proof pass. Multi-home selection, hidden demo/admin removal and exact-installation authority reached independent PASS, after which Sam explicitly authorized a dedicated controlled test relationship and an isolated immutable preview without alias movement. A separate controlled auth identity and clearly synthetic active installation were created under the internal OpenHouse tenant without changing customer records. Controlled local testing exposed and fixed a content API production-column mismatch, a truthfulness bug that treated every unknown system as Solar PV, and an authenticated middleware open redirect via decoded backslashes. The repeated read and operational matrices pass login, durable cookie, exact-home page/API access, foreign denial, assistant, pending service request, alert dismissal, logout and post-logout denial; marked records and limiter buckets were cleaned. The current clean 38-file commit is `61010e2aef0fe45291f61e75226a93caf31dd137`, patch SHA-256 `fd3d59281b6ee85efd2809f6921f4d6e8d6c40023395af1be725e04938ec854b`; four smokes, TypeScript, production build, authenticated redirect regression, diff check and secret scan pass, with final review `deleg_6606139d` pending before preview deployment. Nothing was pushed, merged, deployed or aliased; YC reviewer production remains unchanged. See [[briefs/2026-08-01-openhouse-care-password-and-authority-hardening]].
