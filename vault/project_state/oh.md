...
**Care authentication hardening, 2026-08-01:** Migration 077 committed successfully and its ownership, forced-RLS, privilege, schema and index postconditions pass. The local production login route returns generic `401` for attempts 1–10 then `429` with positive `Retry-After` on attempt 11, with all synthetic rows removed. A PII-free aggregate check found one active email linked to two active installations, so the code now supports zero/one/multiple exact normalized matches through a server-authorized `/care` picker while independently re-authorizing every selected home. The hidden `next`-activated admin/demo branch was removed from homeowner Care login and hard-coded `/care/select` now renders not found. The exact 37-file candidate at SHA-256 `1592f8635b9796bc63fcd9f65e7cf86eabe443befa0576cea5c436d0db9c09ce` received independent PASS from `deleg_2bcb28bc`; four Care smokes, TypeScript, production build, diff checks and secret scan pass. Local checkpoint `6293d61a5d33cae20f80f73126e9f6f1cfa73af6` preserves the reviewed patch byte-for-byte. Release remains blocked on an explicitly authorized serverless candidate plus controlled active-account login, recovery, logout/copied-URL and foreign-installation denial. Nothing was pushed, merged or deployed; the YC reviewer deployment and credentials remain unchanged. See [[briefs/2026-08-01-openhouse-care-password-and-authority-hardening]].
