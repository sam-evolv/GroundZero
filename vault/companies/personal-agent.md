...
- Local compatibility candidates now exist at Aire `944a03e` and Hermes `1004f3e`, but they are unpushed, not loaded by the observed long-running processes, not integrated into the physical source-of-truth branch, and not independently or physically accepted. Direct GitHub comparison at 20:03 IST on 19 August places upstream Hermes `main` at `78ffd71f`, 1,040 commits beyond their `f0c222c` base; the latest verified range materially repairs active-profile session lookup, recent-session retention in a one-profile `ALL` scope, composer and review-agent submission isolation, the sandboxed Desktop preload bridge and unanswered tour-bridge latency, while adding live Desktop update progress and per-provider reasoning-echo opt-in. Treat the local branches as an implemented repair tranche with open integration and acceptance gates, not as a release or current-upstream compatibility proof. See [[project_state/personal-agent]], [[items/personal-agent-hermes-desktop-parity]] and [[items/ops-aire-hermes-upstream-impact-triage]].
