...
- Local compatibility candidates now exist at Aire `944a03e` and Hermes `1004f3e`, but they are unpushed, not loaded by the observed long-running processes, not integrated into the physical source-of-truth branch, and not independently or physically accepted. Direct GitHub comparison at 04:01 IST on 19 August places upstream Hermes `main` at `5dd15872`, 882 commits beyond their `f0c222c` base; the latest verified delta materially adds persistent gateway goal/resume recovery, multi-question clarification across surfaces, agent-applied Desktop layouts, Anthropic structured-output fallback, Bot Mode file attachments and MCP CIMD authentication. Treat the local branches as an implemented repair tranche with open integration and acceptance gates, not as a release or current-upstream compatibility proof. See [[project_state/personal-agent]], [[items/personal-agent-hermes-desktop-parity]] and [[items/ops-aire-hermes-upstream-impact-triage]].
