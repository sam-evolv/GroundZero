...
- Local compatibility candidates now exist at Aire `944a03e` and Hermes `1004f3e`, but they are unpushed, not loaded by the observed long-running processes, not integrated into the physical source-of-truth branch, and not independently or physically accepted. Direct GitHub comparison on 18 August places upstream Hermes `main` at `8911e2e0`, 741 commits beyond their `f0c222c` base; the latest verified delta materially adds Desktop in-app browser navigation, agent-opened and typed URL routing through loopback reach, primary-transport preview resolution including SSH-gateway handling, deep Settings search and clearer TUI references. Treat the local branches as an implemented repair tranche with open integration and acceptance gates, not as a release or current-upstream compatibility proof. See [[project_state/personal-agent]], [[items/personal-agent-hermes-desktop-parity]] and [[items/ops-aire-hermes-upstream-impact-triage]].
