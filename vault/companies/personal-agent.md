...
- Local compatibility candidates remain at Aire `944a03e` and Hermes `1004f3e`; they are unpushed, unintegrated into the physical source-of-truth branch, and not independently or physically accepted. The installed Hermes checkout is clean at `524b062`, 80 commits beyond the prior installed `4a5b6dd`; direct GitHub `main` is `8e77d031`, eight commits beyond the checkout and 1,471 commits beyond compatibility base `f0c222c`. The current supervised gateway started after the checkout commit and returns healthy at the unauthenticated boundary, but process chronology does not prove exact runtime identity, owner acceptance or Aire compatibility. Hermes CLI reporting also said “Up to date” while Git showed the checkout eight commits behind, so that contradiction remains open. The installed and upstream ranges materially affect messaging, memory authority, provider routing, durable Bot Mode rooms, update receipts, old-session editing, macOS launchd discovery and prompt caching. Treat all source/runtime movement as integration input, not an Aire release or physical acceptance. See [[project_state/personal-agent]], [[items/personal-agent-hermes-desktop-parity]] and [[items/ops-aire-hermes-upstream-impact-triage]].
