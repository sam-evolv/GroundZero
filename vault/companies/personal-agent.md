...
- Current physical-iPhone source-of-truth worktree: `/Users/samdonworth/Projects/IrelandGPT-aire-production-ready`. Live Git and route inspection through 18 August found it clean at local, unpublished commit `83e582c` on branch `aire-production-ready-20260810`, with no configured upstream and no remote ref containing the head. The exact candidate is installed on Sam’s iPhone, and Sam directly confirmed that it opens without the false whole-app connection warning. This is startup-warning acceptance only: Profile context still returns `404`, Work still returns `502`, and the Work Receipt detail plus a fresh end-to-end streaming request remain without rendered-screen acceptance. The Hermes userspace Tailscale daemon is running, Serve remains configured for the tailnet-only hostname, and Tailscale's internal resolver now resolves that hostname; ordinary host DNS and direct host-side HTTPS resolution still fail, so tailnet HTTP reachability from the host or iPhone remains unverified. No public route or production deployment was made. Full provenance and remaining gaps are tracked in [[project_state/personal-agent]].
- Local compatibility candidates now exist at Aire `944a03e` and Hermes `1004f3e`, but they are unpushed, not loaded by the observed long-running processes, not integrated into the physical source-of-truth branch, and not independently or physically accepted. Direct GitHub comparison at 12:01 IST on 19 August places upstream Hermes `main` at `13ce0c5c`, 975 commits beyond their `f0c222c` base; the latest verified delta materially repairs release-build profile/agent switching, profile-scoped adapter session keys, non-default-profile Skill Hub installs, remote Files-panel downloads, false-success reporting after Desktop rebuild failure, auto-speak continuity, Bot Mode chat switching, TUI focus repaint, native dependency staging, and live image-model catalog/routing. Treat the local branches as an implemented repair tranche with open integration and acceptance gates, not as a release or current-upstream compatibility proof. See [[project_state/personal-agent]], [[items/personal-agent-hermes-desktop-parity]] and [[items/ops-aire-hermes-upstream-impact-triage]].
