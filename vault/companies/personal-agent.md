...
- Local compatibility candidates now exist at Aire `944a03e` and Hermes `1004f3e`, but they are unpushed, not loaded by the observed long-running processes, not integrated into the physical source-of-truth branch, and not independently or physically accepted. Direct GitHub comparison at 08:06 IST on 19 August places upstream Hermes `main` at `2507bc64`, 942 commits beyond their `f0c222c` base; the latest verified delta materially adds isolated multi-source Desktop routing, restored and atomically published profile switching with bounded mid-dial leases, agent-guided tours, per-chat activity evidence, approval refresh, direct GUI-surface tools, safer Bot Mode group handling, browser controls and per-server MCP OAuth user-agent support. Treat the local branches as an implemented repair tranche with open integration and acceptance gates, not as a release or current-upstream compatibility proof. See [[project_state/personal-agent]], [[items/personal-agent-hermes-desktop-parity]] and [[items/ops-aire-hermes-upstream-impact-triage]].
