...
Give ordinary people the leverage of a capable personal agent without requiring them to install an open-source runtime, use a command line, choose providers, create bots or understand agent infrastructure.

## Founder insight

Sam has experienced the value directly through Hermes plus Ground Zero. The venture productises that experience and uses the agent-assisted build itself as credible proof of capability.
...
- Current physical-iPhone source-of-truth worktree: `/Users/samdonworth/Projects/IrelandGPT-aire-production-ready`. Live Git inspection through 21 August found it clean at local, unpublished commit `83e582c` on branch `aire-production-ready-20260810`, with no configured upstream and no remote ref containing the head. The exact candidate is installed on Sam’s iPhone, and Sam directly confirmed that it opens without the false whole-app connection warning. This is startup-warning acceptance only: the last authenticated candidate probes returned Profile context `404` and Work `502`, while the Work Receipt detail and a fresh end-to-end streaming request remain without rendered-screen acceptance. The restarted Hermes gateway has not been owner-authenticated or physically re-accepted, so those scoped gaps remain open rather than being represented as current responses or closed defects. The Hermes userspace Tailscale daemon is running, Serve remains configured for the tailnet-only hostname, and Tailscale's internal resolver resolves that hostname; ordinary host DNS and direct host-side HTTPS resolution still fail, so tailnet HTTP reachability from the host or iPhone remains unverified. No public route or production deployment was made. Full provenance and remaining gaps are tracked in [[project_state/personal-agent]].
- Local compatibility candidates remain at Aire `944a03e` and Hermes `1004f3e`; they are unpushed, unintegrated into the physical source-of-truth branch, and not independently or physically accepted. The installed Hermes checkout is clean at `524b062`, 80 commits beyond the prior installed `4a5b6dd`; direct GitHub `main` is now `30ccd01`, 31 commits beyond the checkout and 1,494 beyond compatibility base `f0c222c`. The upstream-only range materially affects provider routing, Desktop WSL and idle-renderer behaviour, background-review cancellation, update receipts, macOS gateway restart and cron memory authority. In particular, final upstream commit `ef04d846` explicitly enables `MEMORY.md`, `USER.md` and the memory tool for cron agents after earlier commit `fc9cbc87` removed them, creating an integration-review conflict with [[decisions/2026-08-13-ground-zero-authority-over-hermes-memory]]. The current supervised gateway started after the checkout commit and returns healthy at the unauthenticated boundary, but process chronology does not prove exact runtime identity, owner acceptance or Aire compatibility. Hermes CLI reporting said “Up to date” at tracked upstream `a9ac2c6f` while Git showed the checkout 30 commits behind and direct GitHub was one commit further ahead, so that contradiction remains open. Treat all source/runtime movement as integration input, not an Aire release or physical acceptance. See [[project_state/personal-agent]], [[items/personal-agent-hermes-desktop-parity]] and [[items/ops-aire-hermes-upstream-impact-triage]].
