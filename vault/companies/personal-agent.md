...
- Local compatibility candidates now exist at Aire `944a03e` and Hermes `1004f3e`, but they are unpushed, not loaded by the observed long-running processes, not integrated into the physical source-of-truth branch, and not independently or physically accepted. Direct GitHub comparison on 19 August places upstream Hermes `main` at `a77ee88c`, 841 commits beyond their `f0c222c` base; the latest verified delta materially adds profile-owned and all-gateway session routing, fail-closed profile/gateway/agent switching, rich file-preview routing, native and custom provider discovery, plugin installation/notifications, and Bot Mode hydration, attachments and transcript recovery. Treat the local branches as an implemented repair tranche with open integration and acceptance gates, not as a release or current-upstream compatibility proof. See [[project_state/personal-agent]], [[items/personal-agent-hermes-desktop-parity]] and [[items/ops-aire-hermes-upstream-impact-triage]].
