...
- Local compatibility candidates remain at Aire `944a03e` and Hermes `1004f3e`; they are unpushed, unintegrated into the physical source-of-truth branch, and not independently or physically accepted. The installed Hermes checkout is clean at `524b062`, 80 commits beyond the prior installed `4a5b6dd`; direct GitHub `main` is now release head `fcbd1076`, 50 commits beyond the checkout and 1,513 beyond compatibility base `f0c222c`. The latest upstream-only range affects truthful Desktop Work/progress state, renderer performance, read-only update planning, Bot Mode room persistence, keyless provider routing, model-catalog synchronisation and native compression-summary retention. The earlier upstream cron-memory reversal at `ef04d846` still creates an integration-review conflict with [[decisions/2026-08-13-ground-zero-authority-over-hermes-memory]]. The current supervised gateway started after the checkout commit and returns healthy at the unauthenticated boundary, but process chronology does not prove exact runtime identity, owner acceptance or Aire compatibility. Hermes CLI now correctly reports upstream `fcbd1076` and an update available 50 commits behind, closing the earlier false “Up to date” reporting contradiction. Treat all source/runtime movement as integration input, not an Aire release or physical acceptance. See [[project_state/personal-agent]], [[items/personal-agent-hermes-desktop-parity]] and [[items/ops-aire-hermes-upstream-impact-triage]].
