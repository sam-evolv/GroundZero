...
- Current physical-iPhone source-of-truth worktree: `/Users/samdonworth/Projects/IrelandGPT-aire-production-ready`. Live Git and route inspection on 15 August found it clean at local, unpublished commit `83e582c` on branch `aire-production-ready-20260810`, with no configured upstream and no remote ref containing the head. The exact candidate is installed on Sam’s iPhone, and Sam directly confirmed that it opens without the false whole-app connection warning. This is startup-warning acceptance only: Profile context still returns `404`, Work still returns `502`, and the Work Receipt detail plus a fresh end-to-end streaming request remain without rendered-screen acceptance. The tailnet-only route is live through the Hermes userspace Tailscale socket; no public route or production deployment was made. Full provenance and remaining gaps are tracked in [[project_state/personal-agent]].
