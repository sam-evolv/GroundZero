...
- Local compatibility candidates remain at Aire `944a03e` and Hermes `1004f3e`; they are unpushed, unintegrated into the physical source-of-truth branch, and not independently or physically accepted. The installed Hermes checkout has separately advanced to clean upstream `fdf6f1d4`, and the supervised gateway started after that commit at 20:42 IST on 19 August. This corrects the earlier process-age observation but does not show the local compatibility branch is loaded. Current GitHub `main` is `aebab05f`, 113 commits beyond the running installation and 1,154 commits beyond compatibility base `f0c222c`; the latest verified range materially adds keyless web search for fresh installs, strict stored-provider routing, oversized MCP-result retention warnings, run-budget/stall recovery and Desktop Bot Chat/profile activation repairs. Treat both the local branches and installed-runtime movement as open integration and acceptance inputs, not as an Aire release or physical acceptance. See [[project_state/personal-agent]], [[items/personal-agent-hermes-desktop-parity]] and [[items/ops-aire-hermes-upstream-impact-triage]].
