...
- Local compatibility candidates remain at Aire `944a03e` and Hermes `1004f3e`; they are unpushed, unintegrated into the physical source-of-truth branch, and not independently or physically accepted. The installed Hermes checkout is now clean at upstream `4a5b6dd`, 342 commits beyond the previously recorded installed head `fdf6f1d4`, but the supervised gateway remains the process started at 20:42 IST on 19 August and therefore predates the source advance; no evidence shows the new checkout head is loaded. Current GitHub `main` is `ee000768`, 14 commits beyond the checkout and 1,397 commits beyond compatibility base `f0c222c`. The newly installed range adds Desktop identity, favicon and connector-consent surfaces, while the upstream-only range changes scale-to-zero work accounting and Bot Mode group lifecycle/room behaviour. Treat both local branches and all upstream/runtime movement as open integration and acceptance inputs, not as an Aire release or physical acceptance. See [[project_state/personal-agent]], [[items/personal-agent-hermes-desktop-parity]] and [[items/ops-aire-hermes-upstream-impact-triage]].
