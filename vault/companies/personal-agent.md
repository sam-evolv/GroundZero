...
- Local compatibility candidates remain at Aire `944a03e` and Hermes `1004f3e`; they are unpushed, unintegrated into the physical source-of-truth branch, and not independently or physically accepted. The installed Hermes checkout is clean at `524b062`, 80 commits beyond the prior installed `4a5b6dd`; direct GitHub `main` is now `fd3a783a`, 82 commits beyond the checkout and 1,545 beyond compatibility base `f0c222c`. The latest upstream-only range affects compaction handoff and client projection, authenticated browser-extension control with profile-scoped artifacts and permission gates, provider-parameter retry handling, and Desktop boot-overlay plus tab-strip recovery. The earlier upstream cron-memory reversal at `ef04d846` still creates an integration-review conflict with [[decisions/2026-08-13-ground-zero-authority-over-hermes-memory]]. The supervised gateway returns healthy at the unauthenticated boundary, but process chronology does not prove exact runtime identity, owner acceptance or Aire compatibility. Hermes CLI identifies current upstream `fd3a783a` but reports only 50 commits behind, contradicting the direct immutable comparison of 82; the prior false “Up to date” result is closed, while update-distance reporting is stale again. Treat all source/runtime movement as integration input, not an Aire release or physical acceptance. See [[project_state/personal-agent]], [[items/personal-agent-hermes-desktop-parity]] and [[items/ops-aire-hermes-upstream-impact-triage]].
