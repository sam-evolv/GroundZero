---
title: OpenHouse first UK and US message approval queue
purpose: Copy-ready first-contact drafts for the highest-ranked prospects and channels
status: awaiting-sam-approval-no-outreach-sent
date: 2026-08-05
company_id: openhouse-ai
---

# OpenHouse first UK and US message approval queue

Nothing in this note has been sent. Recheck the recipient's current role and add Sam's preferred signature and website before use.

## 1. Danielle Wells, Stonebond

**Route:** https://www.stonebondproperties.com/contact-us/

**Subject:** Stonebond customer care and exact-home handover

```text
Hi Danielle,

I am one of three people running Longview Estates, a residential developer in Ireland, and I built OpenHouse from working directly across handover and aftercare.

OpenHouse gives each completed home one living record for its documents, systems, warranties and history. Homeowners can ask practical questions grounded in the information for their own home, while the developer sees repeated questions, missing evidence and where customer-care follow-up is still required.

I saw that you lead Customer Care at Stonebond and have a direct remit across quality control and customer satisfaction. I am looking for one UK developer to test a tightly measured, paid pilot on a single scheme or phase, alongside the systems already in place.

Could I get 25 minutes to understand Stonebond's current handover and post-completion workflow? If there is no meaningful gap, we will know quickly.

Sam Donworth
OpenHouse
```

## 2. Les Trott, Weston Homes

**Route:** https://www.weston-homes.com/about-us/contact-us/

**Subject:** Weston quality assurance and customer service

```text
Hi Les,

I help run a residential developer in Ireland and built OpenHouse from the handover and aftercare workflow itself.

It creates one exact-home record for documents, systems, warranties and history. The homeowner gets guidance grounded in their own home, while the developer can see repeated questions, missing information and where customer-service intervention is still needed.

Your role joining Quality Assurance and Customer Service is why I am reaching out. I am not proposing that Weston replace its CRM, warranty or complaints process. I want to test whether one controlled scheme can make the handover record more useful and reveal the information gaps that still create post-completion work.

Would you be open to a 25-minute workflow conversation?

Sam Donworth
OpenHouse
```

## 3. Harriet McVeigh, Hayfield

**Routes:** `customercare@hayfieldhomes.co.uk` and https://uk.linkedin.com/in/harrietdickson

**Subject:** Hayfield handover information and customer care

```text
Hi Harriet,

I am one of three people running Longview Estates in Ireland, where I work directly across residential delivery, handover and homeowner support.

I built OpenHouse because the useful record of a completed home is usually split across handover packs, emails, site teams and customer-care threads. OpenHouse gives each home one living record and lets the developer see the questions and evidence gaps still emerging after completion.

Hayfield already has a dedicated customer-care route, customer charter and New Homes Quality Code process. I am not proposing another complaints tool. I want to test whether the exact-home record can answer more questions from approved information before they become customer-care traffic.

Could I get 25 minutes to understand the current workflow and whether one scheme would justify a measured paid pilot?

Sam Donworth
OpenHouse
```

## 4. Richard Keebler, Garman Homes

**Route:** https://garmanhomes.com/contact-us

**Subject:** Garman's warranty workflow and exact-home records

```text
Hi Richard,

I help run a residential developer in Ireland and built OpenHouse from the post-close homeowner and developer workflow.

OpenHouse creates one exact-home record for documents, systems, warranties and history. Homeowners can get guidance grounded in their own home, while the builder sees repeated questions, missing information and what still requires warranty-team intervention.

Garman's public warranty information shows unusually clear ownership across the team, which is why I am contacting you rather than sending a general technology pitch. I am looking for one US regional builder to test a defined community pilot alongside its existing warranty systems.

Could I get 25 minutes to understand how Garman handles homeowner handoff and warranty intake today?

Sam Donworth
OpenHouse
```

## 5. Christy Reyes, Tilson Homes

**Routes:** https://www.linkedin.com/in/christy-reyes-73526317 and https://www.tilsonhomes.com/contact/

**Subject:** Tilson homeowner handoff and customer care

```text
Hi Christy,

I help run a residential developer in Ireland and built OpenHouse from the handover and aftercare workflow itself.

It gives every completed home one exact record for its documents, systems, warranties and history. Homeowners can get guidance grounded in that record, while the builder sees repeated questions, missing information and where the customer-care team still needs to intervene.

Tilson's build-on-your-land model makes this especially interesting because every site, specification and home differs. A generic home binder is weakest where the exact-home context matters most.

I am looking for one US builder to test a small paid community or completion-cohort pilot alongside the existing warranty process. Would you be open to a 25-minute conversation about Tilson's homeowner handoff and first-year support workflow?

Sam Donworth
OpenHouse
```

## 6. John Galante, Builder Innovator and AE Ventures

**Route:** https://builderinnovator.com/contact/

**Subject:** One regional-builder introduction for a defined post-close pilot

```text
Hi John,

I am one of three people running a residential developer in Ireland and the founder of OpenHouse.

OpenHouse gives each completed home one exact record for documents, systems, warranties and history. Homeowners get guidance grounded in their own home, while the builder sees repeated questions and information gaps across a community.

I am not looking for event sponsorship or a broad US market introduction. I am looking for one or two regional builders actively improving homeowner handoff, warranty intake or post-close customer experience, with the aim of running one tightly measured paid community pilot.

Given the operator network around Builder Innovator, would you be open to a short conversation or point me toward the right person?

Sam Donworth
OpenHouse
```

## 7. Dennis Steigerwalt, Housing Innovation Alliance

**Route:** https://housinginnovationalliance.com/network/join/join/

**Subject:** Regional builder introduction for an exact-home handoff pilot

```text
Hi Dennis,

I help run a residential developer in Ireland and built OpenHouse from working directly across home handover and aftercare.

OpenHouse gives each completed home one exact record for documents, systems, warranties and history. Homeowners get practical guidance grounded in their home, while the builder sees recurring questions, missing evidence and post-close support demand.

The Alliance's work across customer experience, digital transformation, quality and risk is why I am reaching out. I am looking for one or two regional builders willing to assess a tightly measured paid pilot on a single community or phase.

I am not asking the Alliance to endorse OpenHouse. Would you be open to a short conversation or point me toward an operator currently improving homeowner handoff or warranty workflows?

Sam Donworth
OpenHouse
```

## 8. Dan Matta, Schell Brothers

**Route:** https://schellbrothers.com/contact/

**Subject:** Extending Schell's home-specific guide after settlement

```text
Hi Dan,

I help run a residential developer in Ireland and built OpenHouse from the handover and aftercare workflow itself.

I saw that Schell already gives each customer a comprehensive guide specific to their home and continues support through the warranty team after settlement. OpenHouse would turn that handover asset into a living exact-home record, let homeowners ask questions against approved evidence, and show your team where the guide or build record still leaves gaps.

I am looking for one US builder to test a paid pilot on a single community, using the product that already exists rather than starting a technology project.

Could I get 25 minutes to understand Schell's current orientation and warranty workflow?

Sam Donworth
OpenHouse
```

## 9. Terry Nickels, Builder Partnerships

**Route:** https://builderpartnerships.com/contact-us.php

**Subject:** Defined post-close pilot for a private builder

```text
Hi Terry,

I am one of three people running a residential developer in Ireland and the founder of OpenHouse.

OpenHouse gives each completed home one usable record for its documents, systems, warranties and history. It also shows the builder what homeowners ask, what the approved evidence answers and where information is missing.

Builder Partnerships' focus on private mid-tier builders and direct service-provider introductions looks unusually relevant. I am looking for one builder to run a paid pilot on a single community or phase, not a broad advertising campaign.

Could you tell me whether this fits the service-provider route and what evidence or programme cost would be required before introductions are possible?

Sam Donworth
OpenHouse
```

## 10. Kelly Helmes, New Tradition Homes

**Route:** https://www.newtraditionhomes.com/contact-us

**Subject:** New Tradition's homeowner guide and warranty workflow

```text
Hi Kelly,

I help run a residential developer in Ireland and built OpenHouse from working directly across delivery, handover and homeowner support.

New Tradition already gives buyers a homeowner guide, two-year workmanship cover and a defined service-request process. OpenHouse would connect that guidance to the exact systems, selections, documents and warranties in each home, then show the warranty team what customers still cannot find or understand.

Your background across field operations, warranty, purchasing and customer-facing functions is why I am reaching out directly.

Would you be open to a 25-minute conversation about testing one paid community pilot alongside the current process?

Sam Donworth
OpenHouse
```

## 11. Madeline McHugh, Estridge Homes

**Route:** https://www.estridgehomes.com/contact/

**Subject:** Connecting Estridge's sales and HomeLife journeys

```text
Hi Madeline,

I help run a residential developer in Ireland and built OpenHouse from the handover and aftercare workflow.

OpenHouse gives each homeowner one exact record of their documents, systems, warranties and history. Homeowners can ask questions against that evidence, while the builder sees recurring questions and where information is missing.

Your role spanning Sales and Customer Experience makes Estridge especially relevant. The useful test is whether the exact-home record can preserve what was promised and explained before closing, then support the HomeLife warranty relationship afterwards.

Could I get 25 minutes to understand the current journey and whether one neighbourhood would justify a measured paid pilot?

Sam Donworth
OpenHouse
```

## 12. Gene Myers, Thrive Home Builders

**Routes:** `Discover@thrivehomebuilders.com` and https://thrivehomebuilders.com/contact-us/

**Subject:** Making Thrive's high-performance homes legible after move-in

```text
Hi Gene,

I help run a residential developer in Ireland and built OpenHouse from the handover and homeowner-support workflow.

OpenHouse gives each completed home one exact record for documents, systems, warranties and history. Homeowners get guidance grounded in their own equipment and approved information, while the builder sees recurring questions and evidence gaps.

Thrive's healthy and energy-efficient homes make that record particularly valuable. The performance promise depends on residents understanding ventilation, indoor-air-quality features, energy systems and maintenance after move-in.

I am looking for one US builder to test a paid pilot on a single community or house type. Would you be open to a short conversation, or point me to the person who owns customer service and warranty operations?

Sam Donworth
OpenHouse
```

## Suggested first send order

1. Danielle Wells.
2. Les Trott.
3. Harriet McVeigh.
4. Richard Keebler.
5. Dennis Steigerwalt.
6. Dan Matta.
7. Terry Nickels.
8. Kelly Helmes.
9. Madeline McHugh.
10. Gene Myers.

John Galante remains a useful second-wave channel. The Christy Reyes and Tilson message stays parked until her current role is rechecked against a live source.

## Approval choices

Sam can approve:

- all ten first-wave messages;
- named builder messages only;
- UK first;
- US builders first;
- individual edits.

Approval is not assumed from the existence of this note.

## Connected vault notes

- [[briefs/2026-08-05-openhouse-uk-aftercare-prospect-lane]]
- [[briefs/2026-08-05-openhouse-us-builder-channel-prospect-lane]]
- [[briefs/2026-08-05-openhouse-founder-sales-operating-kit]]
- [[briefs/2026-08-05-openhouse-paid-pilot-proposal-template]]

## Notes that link here
_Auto-generated: updated by wiki-refiner_
- [[briefs/2026-08-05-openhouse-irish-named-decision-maker-queue]]
- [[context/index]]
- [[project_state/oh]]

