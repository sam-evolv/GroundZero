...
Release and repair evidence is repeatedly reconstructed by hand across repositories, deployments, runtimes and physical devices. The latest [[project_state/oh]] check proves that OpenHouse `main` advanced directly to `b1629c34` and Vercel served the exact commit, but rendered homeowner acceptance, full type/lint validation and database correctness remained open. The latest [[project_state/personal-agent]] check records a signed and installed Aire candidate at `3556088`, yet fresh streaming-request, visible Home Screen icon and accessibility evidence remain open. These are the same failure mode: build, deployment, installation and user-visible acceptance are different states, but the handoff is assembled manually from scattered evidence.
