...
- Local Aire/Hermes compatibility branches `944a03e` and `1004f3e` implement a repair tranche but remain unpushed, unloaded by the observed processes, unintegrated and without independent or physical acceptance; they do not supersede `83e582c` or close the Profile and Work gaps. Upstream Hermes `main` is now at `5dd15872`, 882 commits beyond their `f0c222c` base, with the latest verified range adding persistent gateway goal/resume recovery, multi-question clarification across surfaces, agent-applied Desktop layouts, Anthropic structured-output fallback, Bot Mode file attachments and MCP CIMD authentication; this is review input, not Aire progress.
