...
- Local Aire/Hermes compatibility branches `944a03e` and `1004f3e` implement a repair tranche but remain unpushed, unloaded by the observed processes, unintegrated and without independent or physical acceptance; they do not supersede `83e582c` or close the Profile and Work gaps. Upstream Hermes `main` is now at `a77ee88c`, 841 commits beyond their `f0c222c` base, with the latest verified range adding profile-owned and all-gateway session routing, fail-closed profile/gateway/agent switching, rich file-preview routing, native and custom provider discovery, plugin installation/notifications, and Bot Mode hydration, attachments and transcript recovery; this is review input, not Aire progress.
