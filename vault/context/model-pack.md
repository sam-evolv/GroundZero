...
- Local Aire/Hermes compatibility branches `944a03e` and `1004f3e` remain unpushed, unintegrated and without independent or physical acceptance; they do not supersede `83e582c` or close the Profile and Work gaps. The installed Hermes checkout has separately advanced to clean upstream `fdf6f1d4`, and the supervised gateway started after that commit at 20:42 IST on 19 August; this corrects the earlier process-age observation but does not show compatibility branch `1004f3e` is loaded. Current upstream `main` is `aebab05f`, 113 commits beyond the running installation and 1,154 commits beyond compatibility base `f0c222c`, with the latest verified range adding keyless web search for fresh installs, strict stored-provider routing, oversized MCP-result retention warnings, run-budget/stall recovery and Desktop Bot Chat/profile activation repairs. This is operational and integration-review input, not Aire progress or physical acceptance.
