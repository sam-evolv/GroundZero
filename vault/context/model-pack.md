...
- Local Aire/Hermes compatibility branches `944a03e` and `1004f3e` implement a repair tranche but remain unpushed, unloaded by the observed processes, unintegrated and without independent or physical acceptance; they do not supersede `83e582c` or close the Profile and Work gaps. Upstream Hermes `main` is now at `daca3869`, 722 commits beyond their `f0c222c` base, with the latest verified range affecting Profile identity, Desktop session recovery, Bot Mode multi-group/thread routing and oversized-result retention; this is review input, not Aire progress.
