...
- Local Aire/Hermes compatibility branches `944a03e` and `1004f3e` remain unpushed, unintegrated and without independent or physical acceptance; they do not supersede `83e582c` or close the Profile and Work gaps. The installed Hermes checkout is clean at upstream `4a5b6dd`, 342 commits beyond the previously recorded `fdf6f1d4`, but the supervised gateway remains the process started at 20:42 IST on 19 August and predates this source advance; no evidence shows the new head is loaded. Current upstream `main` is `18a15a46`, 41 commits beyond the checkout and 1,424 commits beyond compatibility base `f0c222c`. The newly installed range adds Desktop identity, favicon and connector-consent surfaces; the current upstream-only range adds Desktop/Web compiler and mention changes, gateway/relay approval and streamed-final repairs, per-job cron reasoning effort, model-routing changes and Windows update self-tests. This is operational and integration-review input, not Aire progress or physical acceptance.
