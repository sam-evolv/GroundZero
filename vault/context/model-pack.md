...
- Local Aire/Hermes compatibility branches `944a03e` and `1004f3e` remain unpushed, unintegrated and without independent or physical acceptance; they do not supersede `83e582c` or close the Profile and Work gaps. The installed Hermes checkout is clean at `524b062`, 80 commits beyond the prior installed `4a5b6dd`; current upstream `main` is `8e77d031`, eight commits beyond the checkout and 1,471 commits beyond compatibility base `f0c222c`. The supervised gateway now postdates the checkout commit and returns healthy at the unauthenticated boundary, but process chronology does not prove exact runtime identity, owner acceptance or Aire compatibility. Hermes CLI reporting also said “Up to date” while Git showed the checkout eight commits behind. The installed and upstream ranges materially affect messaging, memory authority, provider routing, durable Bot Mode rooms, update receipts, old-session editing, macOS launchd discovery and prompt caching. This is operational and integration-review input, not Aire progress or physical acceptance.
