...
- Local Aire/Hermes compatibility branches `944a03e` and `1004f3e` implement a repair tranche but remain unpushed, unloaded by the observed processes, unintegrated and without independent or physical acceptance; they do not supersede `83e582c` or close the Profile and Work gaps. Upstream Hermes `main` is now at `78ffd71f`, 1,040 commits beyond their `f0c222c` base, with the latest verified range repairing active-profile session lookup, recent-session retention in a one-profile `ALL` scope, composer and review-agent submission isolation, the sandboxed Desktop preload bridge and unanswered tour-bridge latency, while adding live Desktop update progress and per-provider reasoning-echo opt-in; this is review input, not Aire progress.
